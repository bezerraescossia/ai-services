...
- **SC-003**: The top-1 cosine similarity score for a representative in-corpus query meets or exceeds 0.70 — used as a sanity-check threshold, not a tuned production gate. A score below this on an obviously in-topic query indicates the embedding model mismatch mitigation (FR-007) was bypassed or the corpus is severely misaligned.
